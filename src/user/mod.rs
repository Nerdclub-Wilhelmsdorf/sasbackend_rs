...
impl DBUser {
    ...
    pub async fn update_value(
        ...
        let id = self.id.clone(); // Clone the id value
        let updated_user: Option<DBUser> = DB
            .update(("user", id.key().to_owned())) // Use the cloned id value
    ...
    pub async fn update_balance(
        ...
        let id = self.id.clone(); // Clone the id value
        let current_user_state: Option<DBUser> = DB.select(("user", id.key().to_owned())).await?;
